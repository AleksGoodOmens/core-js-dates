...
function getNextFriday(date) {
  ...
  const curDay = dateClone.getDay();

  const daysToNextFriday = (5 - curDay + 7) % 7 || 7;
  dateClone.setDate(dateClone.getDate() + daysToNextFriday);
  return dateClone;
}
...
function getCountWeekendsInMonth(month, year) {
  const daysInMouse = new Date(year, month, 0).getDate();

  let counter = 0;

  for (let k = 1; k <= daysInMouse; k += 1) {
    const currentDay = new Date(year, month - 1, k).getDay();
...
function getWeekNumberByDate(date) {
  const firstJanuary = new Date(date.getFullYear(), 0, 0);

  const difference = date - firstJanuary;
...
function getNextFridayThe13th(date) {
  ...
  return getNextFridayThe13th(new Date(year, month));
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let i = 0; i < totalDays; i += 1) {
    if (workCount < countWorkDays) {
      schedule.push(
        currentDate.toLocaleDateString('ru-Ru').replaceAll('.', '-')
      );
      workCount += 1;
    } else if (offCount < countOffDays) {
      offCount += 1;
    } else {
      workCount = 1;
      offCount = 0;
